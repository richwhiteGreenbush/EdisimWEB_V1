...
ewd_define('EWD_IP_SALT', 'edusim-hifi-world-database');
...
ewd_define('EWD_CANONICAL_BASE', EWD_CANONICAL_ORIGIN . '/hifiworlds/');
...
ewd_define('EWD_APP_OPEN_URL', '../hifi/');

/*
 * THE EDUSIM HIFI WORLDS DATABASE.
 *
 * This is the gallery for Edusim HiFi -- the Babylon.js edition -- and it is a sibling of
 * the World Database next door, not a fork of its data: its own SQLite file, its own world
 * files, its own screenshots. It is served from /hifiworlds/ beside the HiFi app's /hifi/,
 * in the same docroot, for the reason the main gallery sits beside /app/: "Open this world"
 * hands the app a world id and the app fetches the file back out of here, and that fetch
 * has to be same-origin.
 *
 * The link parameter is ?hifiworld=, NOT ?world=. The two editions can be open on one origin
 * at once, and a shared parameter name would let a link meant for one be pasted onto the
 * other, where the id resolves against the WRONG gallery and opens some other world.
 */
ewd_define('EWD_PRODUCT_NAME', 'Edusim HiFi');
ewd_define('EWD_GALLERY_NAME', 'Edusim HiFi Worlds Database');
ewd_define('EWD_WORLD_LINK_PARAM', 'hifiworld');
